...
#          existing copy is older than the latest release, and leaves the
#          sysimage build to the Web UI start, then starts the Web UI.
...
case "$0" in
  *install_webui.sh)
    ...
    # the script ships in tools/ inside the checkout; anchor at the checkout
...
# platform: it checks whether a usable image exists and offers to build one,
# so there is one implementation and one wording instead of an installer
# copy that drifts. SPARLECTRA_BUILD_SYSIMAGE=1 builds it right away, which
# unattended installs use to get the wait over with before the first start.
if [ "${SPARLECTRA_BUILD_SYSIMAGE:-}" = "1" ]; then
  echo "Building the sysimage (this can take a while)..."
  # A failed build must never block the installation: the launcher only uses
  # an image whose metadata validates, so starting without one is safe.
