...
const WEBUI_HELP_TOPICS = Dict(
  ...
  "powsybl_import.hvdc_mode" => (label = "HVDC converters (PowSyBl)", hint = "HVDC converter model of a PowSyBl network: fixed injections reproduce the OpenLoadFlow snapshot; a paired controller is not implemented yet.", doc = "powsybl_import/#powsybl-import-config"),
  "powsybl_import.remote_regulation" => (label = "Remote voltage regulation (PowSyBl)", hint = "Generator regulating another bus: hold_local keeps it PV at its own bus, pq fixes P and Q, remote controls the regulated bus in the outer loop.", doc = "powsybl_import/#powsybl-import-config"),
