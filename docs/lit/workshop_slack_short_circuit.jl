...
#
# **Example 1: the ideal slack.** The default: the connection bus `B1`
# becomes the reference (REF) bus and holds exactly 1.02 pu at 0° while
...
# Reading aid (Example 1): the `SLACK` row at `B1` imports the scheduled
# 60 MW imbalance plus all network losses, and `B1` sits at exactly
# 1.02 pu / 0.000°: the ideal, infinitely stiff busbar.

# ## Scenario 2: non-ideal external-grid source
#
# **Example 2: the non-ideal external-grid source.** With
# `internal_impedance = true` the reference voltage moves to a hidden
...
# Reading aid (Example 2): look at the **first row**. The terminal bus
# `B1` is now an ordinary `PQ` bus at about 1.009 pu and -1.6°, below
# the 1.02 pu
...
# variant degenerates to Example 1.

# ## Scenario 3: distributed slack
#
# **Example 3: the distributed slack.** Examples 1 and 2 answer the
# question "who supplies the missing power?"
...
# Reading aid (Example 3): the bus table now carries the participation
# directly in the columns **`dSl alpha`** and **`Pg eff MW`**: `B3`
# picks up α = 0.6 of the
...
#
# Same network, three grid-connection models: Examples 1 to 3 in one
# table. The losses differ because the
# flow pattern differs: in Example 3 the extra power from `B3` and `B6`
# travels longer paths through the ring, and in Example 2 the feeder
...
#
# **Example 4: the IEC 60909-0 short-circuit sweep.** The external grid is
# more than a voltage boundary condition: its declared
# short-circuit power says how much fault current the superordinate network
# can deliver. The sweep runs on the Example 1 network (`net_slack`), the
# same eight-bus ring. `runShortCircuit!` replaces the operating state by the
...
# Reading aid (Example 4): $I_k''$ is largest at the connection bus `B1`, whose
