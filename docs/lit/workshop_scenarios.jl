...
#                                                                             #src
# file: docs/lit/workshop_scenarios.jl                                        #src
# purpose: Literate.jl source of the scenarios workshop notebook and its      #src
#          Documenter page (scenario task step 7): the scenario model on an   #src
#          SCF case, N-1 expansion, hand-written patch scenarios,             #src
#          runScenarios!, screening off against flag, one screened and one    #src
#          flagged case in detail, and the scenarios block written back into  #src
#          the case file. Every printed number sits next to an @assert, and   #src
#          the extended test suite RUNS this file, so the notebook cannot     #src
#          drift silently. Regenerate the committed outputs with              #src
