...
* Added opt-in MATPOWER metadata import for `bus_name`, user-defined `branch_name`, `branch_kind`, `for001_contingencies`, and proprietary transformer branch-conductance metadata, including stable bus-name mapping and FOR001 contingency branch-index mapping helpers.
...
* Fixed DTF transformer `G` handling so active no-load conductance is represented once as native branch `g_pu` in the transformer PI model instead of as synthetic terminal bus shunts.
* Fixed Sparlectra MATPOWER reimport so proprietary transformer branch conductance metadata restores branch `g_pu` without recreating terminal bus-shunt approximations.
