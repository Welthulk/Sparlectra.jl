...
- Tests on downloaded CGMES deliveries (ENTSO-E conformity package, ReliCapGrid) are removed; the CGMES suites run on synthetic fixtures and on three deliveries exported by Sparlectra itself from the shipped cases sp_case14, sp_case118 and sp_casePST (`data/cgmes_demo`, generated by `tools/gen_cgmes_fixtures.jl`): folder and zip import, SV and source reproduction, the OLTC controller, the PST sign, the service runs and the export-import-export identity.
- The Web UI case selector offers the three CGMES deliveries exported by Sparlectra (`sp_case14_cgmes.zip`, `sp_case118_cgmes.zip`, `sp_casePST_cgmes.zip`), packed into the case cache on first use.
- The sp_case1354 scaling timings are gone from the extended profile; they measured runtime only and decided nothing. The profile is about 50 s shorter.
