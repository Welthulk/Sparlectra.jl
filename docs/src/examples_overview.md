...
Runnable examples live under `examples/`, grouped by topic:
`examples/powerflow/`, `examples/others/` (also the shared suite
infrastructure), `examples/state_estimation/`, `examples/dtf/`, CGMES
tooling in `examples/cgmes/`. Run any example directly:
...
| `exp_external_grid_comparison.jl` | `powerflow` | Grid-connection modeling comparison on an 8-bus ring: ideal slack at two alternative buses, non-ideal external-grid source with internal impedance, and distributed slack, tabulated per bus | powerflow |
...
| `exp_hvdc_b2b_pairing.jl` | `others` | Back-to-back HVDC pairing controller: Stage-0 snapshot vs steerable transfer on a two-island net ([theory](hvdc_back_to_back.md)) | others |
...
| `exp_cgmes_topology_processor.jl` | Node-breaker import without a TP profile: derived bus partition compared bus for bus against the shipped TP on MiniGrid | others |
...
(`standalone` = run directly, not part of a suite; `optional` = skipped
when its inputs are unavailable). The runners print a per-example
pass/fail summary and are exercised by the extended test profile.
