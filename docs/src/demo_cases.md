...
One more shipped case lives outside this table because it is a MATPOWER
file, not an SCF case: `data/mpower/sp_case118.m`, a synthetic 118-bus case
with the cardinalities of the IEEE 118-bus system (118 buses, 186 branches
of which 9 transformers, 54 generators with a good third of synchronous
condensers, 99 loads, three areas, an 11-bus 345 kV backbone over a 138 kV
grid) and its own topology, parameters and names. It is generated by
a generator script that is not part of the package, which validates it with its own Newton-Raphson
power flow (Q-limit enforcement, island check, N-1 sweep), and it stands in
for the downloaded IEEE case in the APSLF and scenarios workshops and their
tests. Several machines sit at a reactive limit in the base case, the base
case keeps 25 percent headroom on every branch, and a handful of N-1
outages island a bus or do not converge, which is what those workshops
need to show.
