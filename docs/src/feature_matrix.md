...
| UPFC (combined shunt + series converter) | ⚠️ | Stationary quadrature composite (`addUpfcControl!`, YAML type `upfc`): one call registers the SSSC series converter (line-flow target, injected-voltage limit) plus the STATCOM shunt converter (remote voltage target, current-based limit) as one named device, both limit characteristics honored per converter side. The warning marker: no series active-power injection, so the phase-shifter degree of freedom of a real UPFC (independent P/Q steering via an injected voltage of arbitrary phase) is out of scope; details on [FACTS Devices](facts.md). |
