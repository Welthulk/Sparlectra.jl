...
| Local browser Web UI | ⚠️ | ❌ | First local-only prototype for PowerFlow forms, result summaries, persistent run history, and safe artifact viewing/download. It binds to loopback only and intentionally has no State Estimation page or public deployment mode. |
