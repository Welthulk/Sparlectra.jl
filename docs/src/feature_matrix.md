...
| UPFC (combined shunt + series converter) | ❌ | Deliberately not implemented: two actuators coupled through a DC-link power balance do not fit the single-actuator outer-loop pattern. The stationary approximation (PST + series controller + STATCOM on one corridor, without the DC-link coupling) and the design note are on [FACTS Devices](facts.md). |
