...
| PowSyBl IIDM import (`PowsyblAdapter`, `read_powsybl_bundle`) | ✅ | XIIDM networks through pypowsybl tables: a `.powsybl` bundle directory written by `tools/powsybl_dump.py` (no Python at run time) or an `.xiidm` file read live through the optional PythonCall extension. Bus-breaker view as buses with retained switches as links, transformers at their current tap, three-winding star buses, HVDC stations as fixed injections, SVC, dangling and tie lines, operational limits; reproduces the OpenLoadFlow solution of the five shipped example files. See [PowSyBl Import](powsybl_import.md). |
