...
| `configuration` | `test/test_configuration_coverage.jl`, `test/test_runner_helpers.jl` | Configuration-key coverage, safe refresh checks including current-iteration start defaults, forwarding checks, start-voltage value-domain validation (including `cgmes_import.start_values` parsing: default `flat`, `sv` accepted, invalid value rejected with the key name), and test-runner output-mode helper checks |
