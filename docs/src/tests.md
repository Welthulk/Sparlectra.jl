...
| `webui` | `test/test_webui.jl` | Public Web UI binding, allowlisted form mapping, complete Markdown-backed form-help coverage, allowlisted documentation-link rewriting, documentation-page whitelisting and traversal rejection, readable artifact/help/docs structure and CSS formatting, service-backed runs, shared logo/header rendering, exact PNG asset routing, result/artifact/history HTML, absence of direct solver calls, and loopback HTTP smoke requests |
