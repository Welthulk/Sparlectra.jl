...
delivery) are third-party data that is not checked in. The CGMES coverage
runs on deliveries exported by Sparlectra itself: `tools/gen_cgmes_fixtures.jl`
imports the shipped cases sp_case14 (OLTC voltage controller), sp_case118
(54 machines with asymmetric Q limits and synchronous condensers) and
sp_casePST (phase-shifting transformer, bus link), solves them with the
rectangular solver and writes their EQ, TP, SSH and SV profiles under
`test/fixtures/cgmes/<case>/` with a fixed header stamp, so a regeneration
is byte identical. The fixture solve keeps Q-limits off: a delivery carries
voltage targets and the SSH operating point, not the solver's active set,
so the SV profile is the solution of the delivery itself and the tests
reproduce it to floating-point noise. The importer reads `minQ`/`maxQ` as
the hull of both sign readings, and the tests pin that contract on the
asymmetric pairs of sp_case118. What the exporter cannot produce stays
uncovered: node-breaker topology (ConnectivityNode and switch states without
a TP profile), boundary sets and their X-node equivalents, DifferenceModel
files, CGMES 3.0, HVDC converters, machine short-circuit attributes, and
the quirks of real deliveries (inconsistent sign conventions, placeholder
values, SV states that disagree with their own model).
