...
code path compiles it for everyone after. Measured on one development
...
Since 2026-09-22 four more synthetic MATPOWER cases ship under `data/mpower`
(`sp_case9`, `sp_case118`, `sp_case300`, `sp_case1354`, generated and
validated by a generator script that is not part of the package), and the
legs that used to gate on the large-case directory run on them instead: the
DC reference table (`sp_case9`, with the generator's own DC solver as the
independent oracle, off-nominal taps included), the screening acceptance
(`sp_case300`), the N-1 parallel identity, the FD-coloring size probe and the
scaling timings (`sp_case1354`), the scenario-engine CSV fixture and both
workshops (`sp_case118`). What still gates on the cache, and says so: the SCF
round-trip legs on real MATPOWER files (numeric bus names, unlimited ratings,
a neutral tap outside its band have no synthetic equivalent yet) and the DTF
deliveries, which cannot be synthesized. The former case14 self-check anchor
is gone: the shipped sp_case14 anchor two lines above it fixes the same
reference.

The test suite runs on checked-in cases only; no test downloads a CGMES
delivery or reads the download cache under `data/CGMES`. The ENTSO-E
conformity package (MicroGrid, MiniGrid, SmallGrid, FullGrid, RealGrid, the
PSEI PST sets) and the ReliCapGrid models (Svedala, the combined seven-area
delivery) are third-party data that is not checked in, and the testsets
that gated on them are removed rather than left as silent skips: the
import, `runpf!` and `compareWithSV` validations, the Stage-2 tap
controllers, the node-breaker topology processor, the PST sign convention,
the placeholder guards, the short-circuit sweeps, the service runs on
fetched ZIPs and the MicroGrid export roundtrip. The CGMES suites run on
synthetic in-memory deliveries. Self-built deliveries exported by Sparlectra
will carry the CGMES importer coverage listed above.
...
`SPARLECTRA_TEST_SHOW_WARNINGS=1` lets the warnings the package logs during a
run reach the console. By default the runner drops `@warn` records raised from
a Sparlectra module: the tests exercise deprecated configuration values,
version-less YAML files, non-case keys handed to `write_case_config` and
advisory topology prechecks on purpose, and the warning is then part of the
behaviour under test, not of the report. Errors, log records from other
packages, and testsets that assert a warning through `@test_logs` are not
affected.
