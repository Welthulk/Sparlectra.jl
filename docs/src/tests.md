...
| `programmatic_api` | `test/test_api.jl` | GUI-ready power-flow API stable unique run IDs, schema versioning, success/failure status, override validation, effective configuration, classic/full log distinction, timing/status summaries, performance, diagnostic, and detailed CSV artifacts (including comma/semicolon quoting and contained diagnostic failure), artifact discovery, Dict/NamedTuple/JSON/YAML serialization, and local service persistent indexing, timestamp fallback, restart recovery, safe single/all-run deletion, run lookup, and artifact/path safety behavior |
