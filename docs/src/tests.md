...
| `programmatic_api` | `test/test_api.jl` | GUI-ready power-flow API stable unique run IDs, schema versioning, success/failure status, override validation, effective configuration, artifact discovery, Dict/NamedTuple/JSON/YAML serialization, and local service run/lookup/artifact safety behavior |
