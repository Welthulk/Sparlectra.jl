...
| `powsybl_extension` | `test/test_powsybl_extension.jl` | The PythonCall extension of the PowSyBl adapter: with PythonCall on the load path and pypowsybl importable, the live read of `ieee14.xiidm` produces the tables of the committed bundle column by column, the manifest fields except timestamp and path, the reference buses byte for byte, and `import_case` takes the `.xiidm` through the extension; without PythonCall or pypowsybl the group prints SKIPPED with the reason and asserts that the generic functions have no method. |
