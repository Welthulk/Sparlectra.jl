...
| `contingency_extended` | `test/test_contingency.jl` | N-1 identity slice on the real case1354pegase (first 60 branches, Web UI case cache required, RAN/SKIPPED stated): serial vs parallel field equality with `max_tasks=1` and auto, plus the `retry_flat_start` invariance on converged cases |
