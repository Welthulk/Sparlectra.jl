...
| `controls` | `test/test_voltage_dependent_control.jl`, `test/test_transformer_phase_shift.jl`, `test/test_tap_controller.jl`, `test/test_tap_changer_model.jl`, `test/test_phase_tap_changer_model.jl`, `test/test_phase_tap_table.jl`, `test/test_3wt_phase_taps.jl` | Voltage-dependent controls, transformer phase-shift control, tap-controller behavior, successful baseline PF preservation when controls are disabled, the `AbstractTapChangerModel`/`convention` field plus `calcRatioTapCorrection`/`calcRatioTapRange` consolidation (Issue #261 Stage 2), the CGMES `PhaseTapChangerModel`/`calcPhaseTapFraction`/`calcPhaseTapAngleRatio`/`calcPhaseTapReactance` formulas plus the DTF importer migration onto them (Issue #261 Stage 3), the tabular `TapTablePoint`/`kind = :tabular`/`calcPhaseTapTable` override path including the formula-vs-table round-trip regression (Issue #261 Stage 4), and `create3WTWindings!`'s `phase_tap_side`/`phase_taps` keywords — existing-behaviour snapshot, single-winding attachment, ratio+phase coexisting on one winding, and validation errors |
