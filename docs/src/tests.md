...
validated by `tools/gen_sp_case118.py` and `tools/gen_sp_cases.py`), and the
...
a neutral tap outside its band have no synthetic equivalent yet), the case14
self-check anchor (a fixed mismatch value of the real file), and the DTF and
CGMES deliveries, which cannot be synthesized.
