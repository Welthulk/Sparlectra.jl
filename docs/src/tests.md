...
| `qlimit_large_case_comparison` | `test/test_qlimit_large_case_comparison.jl` | Offline-safe orchestration tests for the manual large-case Q-limit diagnostic: cached/downloaded/skipped case resolution stubs, live progress capture, classic and robust start-profile schedules, all three Q-limit modes, per-combination row counts, runner exceptions, and CSV/JSON row-shape checks without downloading or solving large cases |
