...
|---|---:|---:|---|---|---|
| `diagnostics.log_effective_config` | Bool | `true` | `true`, `false` | Log merged effective configuration. | true: low |
| `diagnostics.matpower_reference` | Bool | `false` | `true`, `false` | Report MATPOWER reference checks. | true: medium |
| `diagnostics.branch_shift_conventions` | Bool | `false` | `true`, `false` | Report branch shift convention interpretation. | true: low |
| `diagnostics.negative_branch_impedance` | Bool | `false` | `true`, `false` | Report negative branch impedance findings. | true: medium |
| `diagnostics.pv_voltage_references` | Bool | `false` | `true`, `false` | Report PV voltage reference comparisons. | true: medium |
| `diagnostics.residual_clusters` | Bool | `false` | `true`, `false` | Residual-cluster diagnostics. | true: high |
| `diagnostics.nodal_balance_breakdown` | Bool | `false` | `true`, `false` | Nodal power-balance breakdown reporting. | true: high |
| `diagnostics.branch_neighborhood` | Bool | `false` | `true`, `false` | Branch neighborhood analysis report. | true: high |
| `diagnostics.compact_console` | Bool | `true` | `true`, `false` | Compact diagnostic console formatting. | true: low |
| `diagnostics.detailed_log` | Bool | `true` | `true`, `false` | Detailed diagnostic logfile output. | true: medium/high |
