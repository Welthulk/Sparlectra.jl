...
function run_dtf_matpower_export_validation_example_tests()
  ...
  @testset "DTF MATPOWER export validation example" begin
    ...
    outdir = mktempdir()
    cmd = `$(Base.julia_cmd()) --project=$repo $script --dtf-file=$(joinpath(repo, "test", "fixtures", "dtf", "FOR001.DAT")) --for002-file=$(joinpath(repo, "examples", "FOR002.DAT")) --output-dir=$outdir --write-csv=true --write-markdown=true --write-matpower=true --run-outages=true`
    ...
    details = Base.invokelatest(runner, ["--dtf-file=$(joinpath(repo, "test", "fixtures", "dtf", "FOR001.DAT"))", "--for002-file=$(joinpath(repo, "examples", "FOR002.DAT"))", "--output-dir=$(mktempdir())", "--quiet=true", "--write-matpower=true", "--run-outages=true"]; return_details = true)
