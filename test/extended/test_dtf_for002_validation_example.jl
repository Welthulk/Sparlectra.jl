...
function run_dtf_for002_validation_example_tests()
  @testset "native DTF/FOR002 validation example" begin
    ...
    if !(isfile(dtf_file) && isfile(for002_file))
       @info "Skipping full DTF/FOR002 validation example; local external FOR001/FOR002 files are absent." dtf_file for002_file
    ...
    cli_output = read(
      `julia --project=$repo $script --dtf-file=$dtf_file --for002-file=$for002_file --output-dir=$cli_outdir --write-csv=true --write-markdown=true`,
      String,
    )
