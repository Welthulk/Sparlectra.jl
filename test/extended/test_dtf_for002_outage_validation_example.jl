...
function run_dtf_for002_outage_validation_example_tests()
  @testset "native DTF/FOR002 outage validation example" begin
    ...
    detailed = Base.invokelatest(runner, ["--dtf-file=$(joinpath(repo, "test", "fixtures", "dtf", "FOR001.DAT"))", "--for002-file=$(joinpath(repo, "examples", "FOR002.DAT"))", "--output-dir=$outdir", "--write-csv=true", "--write-markdown=true", "--quiet=true"]; return_details=true)
    ...
    cli_output = read(`julia --project=$repo $script --dtf-file=$(joinpath(repo, "test", "fixtures", "dtf", "FOR001.DAT")) --for002-file=$(joinpath(repo, "examples", "FOR002.DAT")) --output-dir=$cli_outdir --write-csv=true --write-markdown=true`, String)
