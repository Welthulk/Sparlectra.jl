...
""")
      ...
      for marker in ("iterations:", "final_mismatch:", "final_mismatch_status:", "final_status:", "final_outcome:")
      ...
      @test occursin("Mismatch summary", diagnostic_log)
      ...
      @test occursin("max_active_power_mismatch:", full_diagnostic_log)
      ...
      final_ok_result = Sparlectra.SparlectraRunResult(
        ...
        true,
        result.raw_result.solution_available,
        result.raw_result.limit_validation_status,
        result.raw_result.final_converged,
        result.raw_result.reason,
        result.raw_result.reason_text,
      ...
      @testset "Q-limit mode metadata and result visibility" begin
        for mode in ("active_set", "classic_simultaneous", "classic_one_at_a_time")
          ...
          if occursin("current_iteration_attempted: true", current_iteration_log)
            @test occursin("current_iteration_attempted: true", run_log)
            @test !occursin("current_iteration_reason: disabled", run_log)
            if isfile(diagnose_path)
              diagnose_log = read(diagnose_path, String)
              @test occursin("current_iteration_attempted: true", diagnose_log)
