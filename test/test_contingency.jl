...
function run_contingency_extended_tests()
  @testset "N-1 identity on sp_case1354 (extended)" begin
    ...
    # directory paths in tests); without the
    # variable the testset states SKIPPED explicitly, without a silent
    # counting pass
    # since the shipped data/mpower/sp_case1354.m (synthetic 1354-bus grid,
    # 2040 branches, generated by a case generator that is not shipped) the identity check
    # runs on every checkout; the former case1354pegase leg needed the
    # large-case directory
    case_path = abspath(joinpath(dirname(@__DIR__), "data", "mpower", "sp_case1354.m"))
    @test isfile(case_path)
    net = createNetFromMatPowerFile(filename = case_path, flatstart = false, enable_pq_gen_controllers = true, bus_shunt_model = :admittance, matpower_shift_sign = 1.0, matpower_shift_unit = :deg, matpower_ratio = :normal, tap_changer_model = :ideal)
