...
#          at tools/install_webui.sh, which installs Julia and fetches the latest
#          tagged Sparlectra release before starting the Web UI.
#          The sysimage decision (check, offer to build, relaunch through the
#          image) lives in start_webui.jl so that Linux, macOS and Windows go
#          through the same code; this script only launches Julia.
...
if ! command -v julia >/dev/null 2>&1; then
  ...
  echo "Run   $DIR/tools/install_webui.sh   instead:"
