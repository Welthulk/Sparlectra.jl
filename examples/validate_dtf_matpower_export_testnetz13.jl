...
function parse_cli(args)
  opt = Dict{String,Any}(
    "dtf-file" => isfile(joinpath(@__DIR__, "..", "test", "fixtures", "dtf", "FOR001.DAT")) ? joinpath(@__DIR__, "..", "test", "fixtures", "dtf", "FOR001.DAT") : "test/fixtures/dtf/FOR001.DAT",
...
function _metadata_summary(matpower_path)
  ...
  return (
    ...
    metadata_complete = mpc.bus_name !== nothing && length(mpc.bus_name) == 13 &&
      mpc.branch_name !== nothing && length(mpc.branch_name) == 27 &&
      mpc.branch_kind !== nothing && length(mpc.branch_kind) == 27 &&
      length(contingencies) >= 2,
  )
end
...
function _bus_rows(scenario, outage_index, outage_label, native, rt)
  ...
  for i in eachindex(native.nodeVec)
    n = native.nodeVec[i]; r = rt.nodeVec[i]
    push!(rows, (scenario = scenario, outage_index = outage_index, outage_label = outage_label,
      bus_index_native = i, bus_index_roundtrip = i, bus_name_native = n.comp.cName, bus_name_roundtrip = r.comp.cName,
      baseKV_native = n.comp.cVN, baseKV_roundtrip = r.comp.cVN, vm_pu_native = n._vm_pu, vm_pu_roundtrip = r._vm_pu,
      d_vm_pu = r._vm_pu - n._vm_pu, vm_kV_native = n.comp.cVN*n._vm_pu, vm_kV_roundtrip = r.comp.cVN*r._vm_pu,
      d_vm_kV = r.comp.cVN*r._vm_pu - n.comp.cVN*n._vm_pu, va_deg_native = n._va_deg, va_deg_roundtrip = r._va_deg,
      d_va_deg = r._va_deg - n._va_deg, bus_type_native = Sparlectra.toString(n._nodeType), bus_type_roundtrip = Sparlectra.toString(r._nodeType), notes = ""))
  end
  return rows
end
...
function _branch_rows(scenario, outage_index, outage_label, native, rt, outaged_idx)
  ...
  for i in eachindex(native.branchVec)
    ...
    rb = rt.branchVec[j]
    pf = b.fBranchFlow === nothing ? missing : b.fBranchFlow.pFlow; qf = b.fBranchFlow === nothing ? missing : b.fBranchFlow.qFlow
    pt = b.tBranchFlow === nothing ? missing : b.tBranchFlow.pFlow; qt = b.tBranchFlow === nothing ? missing : b.tBranchFlow.qFlow
    rpf = rb.fBranchFlow === nothing ? missing : rb.fBranchFlow.pFlow; rqf = rb.fBranchFlow === nothing ? missing : rb.fBranchFlow.qFlow
    rpt = rb.tBranchFlow === nothing ? missing : rb.tBranchFlow.pFlow; rqt = rb.tBranchFlow === nothing ? missing : rb.tBranchFlow.qFlow
    isout = outaged_idx !== nothing && i == outaged_idx
    push!(rows, (scenario = scenario, outage_index = outage_index, outage_label = outage_label,
      branch_index_native = i, branch_index_roundtrip = j, branch_name_native = native_name, branch_name_roundtrip = _branch_name(rt, j),
      from_bus_native = _bname(native, b.fromBus), to_bus_native = _bname(native, b.toBus), from_bus_roundtrip = _bname(rt, rb.fromBus), to_bus_roundtrip = _bname(rt, rb.toBus),
      r_pu_native = b.r_pu, r_pu_roundtrip = rb.r_pu, d_r_pu = rb.r_pu-b.r_pu, x_pu_native = b.x_pu, x_pu_roundtrip = rb.x_pu, d_x_pu = rb.x_pu-b.x_pu,
      b_pu_native = b.b_pu, b_pu_roundtrip = rb.b_pu, d_b_pu = rb.b_pu-b.b_pu, g_pu_native = b.g_pu, g_pu_roundtrip = rb.g_pu, d_g_pu = rb.g_pu-b.g_pu,
      branch_kind_native = _branch_kind(native, i), branch_kind_roundtrip = _branch_kind(rt, j),
      exported_tap = _branch_kind(native, i) == "T" ? b.ratio : 0.0, ratio_native = b.ratio, ratio_roundtrip = rb.ratio, d_ratio = rb.ratio-b.ratio,
      status_native = b.status, status_roundtrip = rb.status, p_from_native_MW = pf, p_from_roundtrip_MW = rpf, d_p_from_MW = (pf isa Missing || rpf isa Missing || isout) ? missing : rpf-pf,
      q_from_native_MVar = qf, q_from_roundtrip_MVar = rqf, d_q_from_MVar = (qf isa Missing || rqf isa Missing || isout) ? missing : rqf-qf,
      p_to_native_MW = pt, p_to_roundtrip_MW = rpt, d_p_to_MW = (pt isa Missing || rpt isa Missing || isout) ? missing : rpt-pt,
      q_to_native_MVar = qt, q_to_roundtrip_MVar = rqt, d_q_to_MVar = (qt isa Missing || rqt isa Missing || isout) ? missing : rqt-qt,
      is_outaged_branch = isout, notes = isout ? "outaged branch; flow deltas excluded" : ""))
  end
  return rows
end
...
function _gen_rows(scenario, outage_index, outage_label, native, rt)
  rows = NamedTuple[]
  for i in eachindex(native.nodeVec)
    ng = _bus_generation_load(native, i); rg = _bus_generation_load(rt, i)
    (ng.has_generator || ng.has_load || rg.has_generator || rg.has_load) || continue
    nn = native.nodeVec[i]; rn = rt.nodeVec[i]
    push!(rows, (scenario = scenario, outage_index = outage_index, outage_label = outage_label, bus_name_native = nn.comp.cName, bus_name_roundtrip = rn.comp.cName,
      bus_index_native = i, bus_index_roundtrip = i, pg_native_MW = ng.pg_res, pg_roundtrip_MW = rg.pg_res, d_pg_MW = rg.pg_res-ng.pg_res,
      qg_native_MVar = ng.qg_res, qg_roundtrip_MVar = rg.qg_res, d_qg_MVar = rg.qg_res-ng.qg_res, p_load_native_MW = ng.pl, p_load_roundtrip_MW = rg.pl,
      d_p_load_MW = rg.pl-ng.pl, q_load_native_MVar = ng.ql, q_load_roundtrip_MVar = rg.ql, d_q_load_MVar = rg.ql-ng.ql,
      is_slack_native = nn._nodeType == Sparlectra.Slack, is_slack_roundtrip = rn._nodeType == Sparlectra.Slack, is_regulating_native = ng.is_regulating, is_regulating_roundtrip = rg.is_regulating, notes = ""))
  end
  return rows
end
...
function _scenario(case, opt, scenario, outage, outaged_idx)
  ...
  metric = (scenario = scenario, outage_index = outage === nothing ? missing : outage.index, outage_label = outage === nothing ? "" : _outage_label(outage),
    native_converged = npf.converged, roundtrip_converged = rpf.converged, native_iterations = npf.iterations, roundtrip_iterations = rpf.iterations,
    native_final_mismatch = npf.final_mismatch, roundtrip_final_mismatch = rpf.final_mismatch, max_abs_d_vm_pu = _maxabs(buses, :d_vm_pu), max_abs_d_vm_kV = _maxabs(buses, :d_vm_kV), max_abs_d_va_deg = _maxabs(buses, :d_va_deg),
    max_abs_branch_d_p_MW = max(_maxabs(branches, :d_p_from_MW), _maxabs(branches, :d_p_to_MW)), max_abs_branch_d_q_MVar = max(_maxabs(branches, :d_q_from_MVar), _maxabs(branches, :d_q_to_MVar)),
    max_abs_branch_r_pu = _maxabs(branches, :d_r_pu), max_abs_branch_x_pu = _maxabs(branches, :d_x_pu), max_abs_branch_b_pu = _maxabs(branches, :d_b_pu), max_abs_ratio = _maxabs(branches, :d_ratio),
    max_abs_branch_g_pu_native = maximum([abs(br.g_pu) for br in native.branchVec]; init = 0.0), count_nonzero_branch_g_pu_native = count(br -> abs(br.g_pu) > 1e-12, native.branchVec),
    native_total_bus_Gs = native_shunts.Gs, native_total_bus_Bs = native_shunts.Bs, exported_total_bus_Gs = native_shunts.Gs, exported_total_bus_Bs = native_shunts.Bs,
    roundtrip_total_bus_Gs = roundtrip_shunts.Gs, roundtrip_total_bus_Bs = roundtrip_shunts.Bs,
    bus_count_native = length(native.nodeVec), bus_count_roundtrip = length(rt.nodeVec), branch_count_native = length(native.branchVec), branch_count_roundtrip = length(rt.branchVec),
    generator_count_native = length(native.prosumpsVec), generator_count_roundtrip = length(rt.prosumpsVec), status_match = _statuses(native) == _statuses(rt),
...
function run_validation(args = ARGS; return_details::Bool = false)
  opt = parse_cli(args); return_details |= opt["details"]
  ...
  metrics = [r.metric for r in results]; buses = reduce(vcat, [r.bus_rows for r in results]; init = NamedTuple[]); branches = reduce(vcat, [r.branch_rows for r in results]; init = NamedTuple[]); gens = reduce(vcat, [r.generator_rows for r in results]; init = NamedTuple[])
  written = String[]
  if opt["write-csv"]
    for (fn, rows) in [("dtf_matpower_export_metrics.csv", metrics), ("dtf_matpower_export_bus_comparison.csv", buses), ("dtf_matpower_export_branch_comparison.csv", branches), ("dtf_matpower_export_generator_comparison.csv", gens)]
      p = joinpath(opt["output-dir"], fn); write_csv(p, rows); push!(written, p)
    end
  end
  if opt["write-markdown"]
    p = joinpath(opt["output-dir"], "dtf_matpower_export_summary.md"); _write_summary(p, opt, results); push!(written, p)
  end
