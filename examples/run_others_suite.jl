...
const SUITE_SPECS = ExampleSpec[
  ...
  ExampleSpec(name = "facts_limit_modes", file = "others/exp_facts_limit_modes.jl", purpose = "FACTS limit characteristics side by side (issue #297 Drafts A/E/F): constant-Q box vs STATCOM (V*S_max) vs SVC (V^2*B) on one sagging corridor, plus SSSC injected-voltage window vs TCSC fixed window"),
  ExampleSpec(name = "facts_base_impedance", file = "others/exp_facts_base_impedance.jl", purpose = "equipment impedance vs FACTS operating point (issue #329): after a full-UPFC control run, short circuit and MATPOWER export read the physical base impedance and match the equipment net; restoreBaseImpedances!/clearUpfcFullControllers!"),
  ...
  ExampleSpec(name = "hvdc_meshed_ac_tie", file = "others/exp_hvdc_meshed_ac_tie.jl", purpose = "HVDC pair in parallel to an AC tie: one reference per synchronous island, setpoint pair as parallel PQ path, island_feed rejected"),
  ExampleSpec(name = "open_terminal_line", file = "others/exp_open_terminal_line.jl", purpose = "one-sided open line (r0.9.10): full charging draw at the closed bus, r loss of the charging current, Ferranti rise at the open end"),
