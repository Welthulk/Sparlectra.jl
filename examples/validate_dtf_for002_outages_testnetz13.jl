...
function parse_cli(args)
  ...
  opt = Dict{String,Any}(
    "dtf-file" => normpath(dtf_default), "for002-file" => joinpath(@__DIR__, "FOR002.DAT"),
    "output-dir" => joinpath(@__DIR__, "_out", "dtf_for002_native_outages"),
    "tol" => 1e-8, "max-iter" => 50, "method" => "rectangular",
    "write-csv" => true, "write-markdown" => true, "quiet" => false,
    "strict" => false, "details" => false, "print-summary" => true)
  for arg in args
    arg == "--quiet" && (opt["quiet"] = true; continue)
    startswith(arg, "--") || throw(ArgumentError("unsupported argument: $arg"))
    k, v = split(arg[3:end], "="; limit = 2)
    if k == "tol"; opt[k] = parse(Float64, v)
    elseif k == "max-iter"; opt[k] = parse(Int, v)
    elseif k in ("write-csv", "write-markdown", "strict", "quiet", "details", "print-summary"); opt[k] = _parse_bool(v)
    else; opt[k] = v
    end
  end
  return opt
...
function _maxabs(rows, field)
  ...
  for r in rows
    ...
    fv = Float64(v); isfinite(fv) && push!(vals, abs(fv))
  end
...
function _comparison_rows(case, net, ref, outage_index, outage_label, outaged_idx)
  ...
  for db in case.buses
    ...
    push!(gen_rows, (outage_index=outage_index, outage_label=outage_label, bus_name=db.name, dtf_bus_type=db.bus_type, model_bus_type_effective=Sparlectra.toString(node._nodeType), for002_pg_MW=fb.p_gen_MW, model_pg_specified_MW=gl.pg_spec, model_pg_result_MW=solved_pg, d_pg_result_vs_for002_MW=solved_pg-fb.p_gen_MW, for002_qg_MVar=fb.q_gen_MVar, model_qg_specified_MVar=gl.qg_spec, model_qg_result_MVar=solved_qg, d_qg_result_vs_for002_MVar=solved_qg-fb.q_gen_MVar, qmin_MVar=something(db.qmin_mvar, missing), qmax_MVar=something(db.qmax_mvar, missing), is_regulating=gl.is_regulating || flags.is_pv, is_slack=flags.is_slack, has_generator=gl.has_generator, has_load=gl.has_load, p_load_MW=gl.pl, q_load_MVar=gl.ql, notes=flags.is_slack ? "slack bus: solved P/Q inferred from branch KCL plus load" : (gl.is_regulating || flags.is_pv ? "PV/regulating bus: result P/Q inferred from branch KCL plus load" : "")))
  end
  rd = branch_ref_dict(ref); branch_rows = NamedTuple[]
  for (idx, br) in enumerate(net.branchVec)
    meta = net.matpower_branch_metadata[idx]; dtfb = case.branches[idx]; nr = uppercase(strip(String(meta.parallel_id)))
    rf = get(rd, (_norm_name(dtfb.from), _norm_name(dtfb.to), nr), missing); rt = get(rd, (_norm_name(dtfb.to), _norm_name(dtfb.from), nr), missing)
    pf = br.fBranchFlow === nothing || br.fBranchFlow.pFlow === nothing ? missing : br.fBranchFlow.pFlow; qf = br.fBranchFlow === nothing || br.fBranchFlow.qFlow === nothing ? missing : br.fBranchFlow.qFlow
    pt = br.tBranchFlow === nothing || br.tBranchFlow.pFlow === nothing ? missing : br.tBranchFlow.pFlow; qt = br.tBranchFlow === nothing || br.tBranchFlow.qFlow === nothing ? missing : br.tBranchFlow.qFlow
    push!(branch_rows, (outage_index=outage_index, outage_label=outage_label, branch_index=idx, branch_label=meta.orig_name, branch_kind=string(meta.dtf_kind), voltage_level_index=meta.voltage_level_index, parallel_id=meta.parallel_id, from_bus=dtfb.from, to_bus=dtfb.to, is_outaged_branch=idx==outaged_idx, branch_status=br.status, u_ref_kV=meta.u_ref_kV, r_pu=br.r_pu, x_pu=br.x_pu, b_pu=br.b_pu, ratio=br.ratio, for002_p_from_MW=rf===missing ? missing : rf.p_MW, model_p_from_MW=pf, d_p_from_MW=rf===missing || pf===missing ? missing : pf-rf.p_MW, for002_q_from_MVar=rf===missing ? missing : rf.q_MVar, model_q_from_MVar=qf, d_q_from_MVar=rf===missing || qf===missing ? missing : qf-rf.q_MVar, for002_p_to_MW=rt===missing ? missing : rt.p_MW, model_p_to_MW=pt, d_p_to_MW=rt===missing || pt===missing ? missing : pt-rt.p_MW, for002_q_to_MVar=rt===missing ? missing : rt.q_MVar, model_q_to_MVar=qt, d_q_to_MVar=rt===missing || qt===missing ? missing : qt-rt.q_MVar))
  end
  kcl_base = branch_kcl_rows(net, case, ref)
  kcl_rows = [(outage_index=outage_index, outage_label=outage_label, bus_name=r.bus_name, dtf_bus_type=r.dtf_bus_type, is_slack=r.is_slack, is_pv=r.is_pv, is_pq=r.is_pq, for002_p_net_MW=r.for002_p_net_MW, for002_q_net_MVar=r.for002_q_net_MVar, model_result_p_net_MW=r.model_result_p_net_MW, model_result_q_net_MVar=r.model_result_q_net_MVar, branch_kcl_p_net_MW=r.branch_kcl_p_net_MW, branch_kcl_q_net_MVar=r.branch_kcl_q_net_MVar, d_model_result_vs_for002_p_MW=r.d_model_result_vs_for002_p_MW, d_model_result_vs_for002_q_MVar=r.d_model_result_vs_for002_q_MVar, d_branch_kcl_vs_for002_p_MW=r.d_branch_kcl_vs_for002_p_MW, d_branch_kcl_vs_for002_q_MVar=r.d_branch_kcl_vs_for002_q_MVar, notes=r.notes) for r in kcl_base]
  res_rows = [(outage_index=outage_index, outage_label=outage_label, bus_name=r.bus_name, forced_for002_vm_pu=r.forced_for002_vm_pu, forced_for002_va_deg=r.forced_for002_va_deg, ybus_calc_p_MW=r.ybus_calc_p_MW, ybus_calc_q_MVar=r.ybus_calc_q_MVar, for002_bus_net_p_MW=r.for002_bus_net_p_MW, for002_bus_net_q_MVar=r.for002_bus_net_q_MVar, d_p_MW=r.d_p_MW, d_q_MVar=r.d_q_MVar) for r in for002_state_residual_rows(net, case, ref)]
...
function _write_markdown(path, opt, case, for002_scenarios, matching_rows, metrics_rows, bus_rows, branch_rows, residual_rows)
  open(path, "w") do io
    ...
    for m in metrics_rows
      ...
      for (title, rows, field, unit) in [("Top voltage deviations", bus_rows, :d_vm_kV, "kV"), ("Top branch P deviations", branch_rows, :d_p_from_MW, "MW"), ("Top branch Q deviations", branch_rows, :d_q_from_MVar, "MVar"), ("Top state residual P deviations", residual_rows, :d_p_MW, "MW"), ("Top state residual Q deviations", residual_rows, :d_q_MVar, "MVar")]
        println(io, "\n### ", title, "\n")
...
function run_validation(args = ARGS; return_details::Bool = false)
  opt = parse_cli(args); return_details = return_details || opt["details"]
  ...
  matching_rows = NamedTuple[]; metrics_rows = NamedTuple[]; all_bus = NamedTuple[]; all_gen = NamedTuple[]; all_branch = NamedTuple[]; all_kcl = NamedTuple[]; all_res = NamedTuple[]; outage_results = NamedTuple[]
  for outage in case.outages
    label = _outage_label(outage); branch_matches = _find_branch(case, outage); scen_matches = _find_for002_scenario(for002_scenarios, outage)
    ...
    push!(matching_rows, (outage_index=outage.index, outage_label=label, outage_kind=string(outage.kind), voltage_level_index=outage.voltage_level_index, parallel_id=outage.parallel_id, from_bus=outage.from, to_bus=outage.to, matched_branch_index=branch_idx, matched_for002_scenario_index=scen_idx, matched_for002_heading=scen_idx===missing ? missing : for002_scenarios[scen_idx].raw_heading, match_status=status, notes=notes))
    ...
    append!(all_bus, bus_rows); append!(all_gen, gen_rows); append!(all_branch, branch_rows); append!(all_kcl, kcl_rows); append!(all_res, res_rows)
    metric = (outage_index=outage.index, outage_label=label, converged=pfstatus==0, iterations=iters, final_mismatch=_final_mismatch(net), max_abs_d_vm_kV=_maxabs(bus_rows, :d_vm_kV), max_abs_d_vm_pu=_maxabs(bus_rows, :d_vm_pu), max_abs_d_va_deg=_maxabs(bus_rows, :d_va_deg), max_abs_branch_d_p_MW=max(_maxabs(branch_rows, :d_p_from_MW), _maxabs(branch_rows, :d_p_to_MW)), max_abs_branch_d_q_MVar=max(_maxabs(branch_rows, :d_q_from_MVar), _maxabs(branch_rows, :d_q_to_MVar)), max_abs_d_pg_MW=_maxabs(gen_rows, :d_pg_result_vs_for002_MW), max_abs_d_qg_MVar=_maxabs(gen_rows, :d_qg_result_vs_for002_MVar), max_abs_state_residual_p_MW=_maxabs(res_rows, :d_p_MW), max_abs_state_residual_q_MVar=_maxabs(res_rows, :d_q_MVar), max_abs_branch_kcl_vs_for002_p_MW=_maxabs(kcl_rows, :d_branch_kcl_vs_for002_p_MW), max_abs_branch_kcl_vs_for002_q_MVar=_maxabs(kcl_rows, :d_branch_kcl_vs_for002_q_MVar))
  ...
  if opt["write-csv"]
    for (filename, rows) in [("dtf_outage_matching.csv", matching_rows), ("dtf_outage_metrics.csv", metrics_rows), ("dtf_outage_bus_comparison.csv", all_bus), ("dtf_outage_generator_comparison.csv", all_gen), ("dtf_outage_branch_comparison.csv", all_branch), ("dtf_outage_bus_kcl_comparison.csv", all_kcl), ("dtf_outage_state_residual.csv", all_res)]
      path = joinpath(opt["output-dir"], filename); write_csv(path, rows); push!(written_files, path)
    end
  end
  ...
  return return_details ? (case=case, for002_outage_scenarios=for002_scenarios, matching_rows=matching_rows, outage_results=outage_results, metrics_rows=metrics_rows, bus_rows=all_bus, generator_rows=all_gen, branch_rows=all_branch, kcl_rows=all_kcl, residual_rows=all_res, output_dir=opt["output-dir"], written_files=written_files) : result
end
