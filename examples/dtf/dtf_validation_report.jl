...
# Date: 2026-07-13
# file: examples/dtf/dtf_validation_report.jl
...
include(joinpath(@__DIR__, "dtf_validation_base.jl"))
...
function main()
  print_example_banner("examples/dtf/dtf_validation_report.jl", "runs native DTF/FOR002 validation across cases A-E and writes a combined loss/voltage-transfer summary report")
  ...
  md_path = joinpath(outdir, "dtf_validation_summary.md")
  open(md_path, "w") do io
    ...
    for c in CASES
      ...
      write_named_csv(joinpath(outdir, "dtf_case_$(c.id).csv"), [selected_row])
    end
  end
  write_named_csv(joinpath(outdir, "dtf_validation_summary.csv"), summary_rows)
  ...
  println("Wrote ", joinpath(outdir, "dtf_validation_summary.csv"))
  return nothing
end
