...
2. Subtract outgoing terminal branch powers to get the link right-hand side `b`.
3. Build oriented incidence matrix `A` for active links.
4. Solve per connected link component (BFS) using `pinv(A_component) * b_component`.
5. If `sum(b_component)` is not near zero, distribute the residual uniformly
   before solving so the component system becomes consistent.
6. Write resulting link P/Q flows and derive terminal currents from |S| and V_LL.
...
function calcLinkFlowsKCL!(net::Net; tol::Float64 = 1e-6)
  ...
  # 3) Remaining nodal balance that must be carried by links.
  ...
  # 4) Split by connected components in the link graph for robust solving.
  ...
  for b0 in 1:nbus
    ...
    # 5) Enforce component solvability condition sum(b)=0.
    ...
    # 6) Least-squares solve. `pinv` also handles singular meshed components
  ...
  # 7) Reset all link outputs, then write back solved values for active links.
  ...
  for (j, l) in enumerate(active)
    ...
    # 8) Three-phase current magnitudes per terminal:
