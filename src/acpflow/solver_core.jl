...
# Date: 2024-06-01
# file: src/acpflow/solver_core.jl
...
## per-unit bound as a voltage quantity and carry no watt equivalent.
function format_tolerance_physical(tol::Float64, base_mva::Float64)::String
  watts = tol * base_mva * 1.0e6
  scaled, unit = watts < 1.0e3 ? (watts, "W") : watts < 1.0e6 ? (watts / 1.0e3, "kW") : (watts / 1.0e6, "MW")
  value = round(scaled; sigdigits = 3)
  shown = isinteger(value) && abs(value) < 1.0e15 ? string(Int(value)) : string(value)
    ...
    if allow_pinv && _is_linear_singularity_error(e)
      try
        ...
        if _is_linear_fallback_error(qr_error)
          max(size(A)...) <= svd_max_n || rethrow(qr_error)
