...
function _postprocess_sparlectra_result!(result::SparlectraRunResult, cfg::SparlectraConfig; emit_output::Bool = true)
  ...
  if emit_output && out_cfg.logfile_results !== :off
    ...
    _perf_profile_time!(result.performance_profile, :result_output) do
      printACPFlowResults(result.net, result.elapsed_s, result.iterations, cfg.powerflow.tol, false, ""; converged = result.final_converged, solver = result.diagnostics.solver, solver_time_s = result.solver_elapsed_s, result_mode = _sparlectra_result_mode(result.net, out_cfg), max_rows = max_rows, condition_number = out_cfg.condition_number)
    end
  end
  return result
end
