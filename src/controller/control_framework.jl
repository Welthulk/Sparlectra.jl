...
# Date: 26.5.2026
# file: src/controller/control_framework.jl
...
"""
Supertype of the outer-loop controllers the control framework iterates
(tap, shunt, machine, series reactance, UPFC, HVDC pair).
"""
...
"""
Outer-loop controller settings of a run: iteration budget, tolerance and
logging of the controller loop.
"""
...
"""
Outcome of a controller-driven run: convergence, outer iterations and the
per-controller updates that were applied.
"""
...
"""
    collect_outer_controllers(net) -> Vector{AbstractOuterController}

Every outer-loop controller attached to the network, across all controller
families.
"""
collect_outer_controllers(net::Any)::Vector{AbstractOuterController} = AbstractOuterController[_tap_controllers(net)..., _machine_controllers(net)..., _shunt_controllers(net)..., _series_reactance_controllers(net)..., _hvdc_pair_controllers(net)..., _upfc_full_controllers(net)...]
...
"""
    latest_control_result(net) -> Union{Nothing,ControlRunResult}

The controller result of the network's last controlled run.
"""
