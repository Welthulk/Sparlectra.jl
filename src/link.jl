...
"""
mutable struct BusLink <: AbstractComponent
  cID::String
  cName::String
  cTyp::ComponentTyp
  cVN::Float64
  ...
  function BusLink(; linkIdx::Int, fromBus::Int, toBus::Int, status::Int = 1, cID::String = string(linkIdx), cName::String = "Link_$(linkIdx)", cVN::Float64 = 1.0)
    @assert fromBus != toBus "fromBus and toBus must differ"
    @assert status in (0, 1) "status must be 0 or 1"
    new(cID, cName, LinkC, cVN, linkIdx, fromBus, toBus, status, nothing, nothing, nothing, nothing)
  end

  function Base.show(io::IO, l::BusLink)
    print(io, "BusLink(")
    print(io, "ID=", l.cID, ", ")
    print(io, "Name=", l.cName, ", ")
    print(io, "Typ=", l.cTyp, ", ")
