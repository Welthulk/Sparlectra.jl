...
function run_acpflow(;
  ...
  if ext in (".m", ".jl")
    ...
    if opt_flatstart && (Symbol(flatstart_voltage_mode) != :classic || Symbol(flatstart_angle_mode) != :classic)
      mpc_init = MatpowerIO.read_case(in_path; legacy_compat = true)
