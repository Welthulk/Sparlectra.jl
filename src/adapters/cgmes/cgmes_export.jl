...
# file: src/adapters/cgmes/cgmes_export.jl
...
struct CGMESContext
  ...
  tapCtrlRecs::Vector{NamedTuple}         # one record per exported tap-changer control group (#322)
...
# per active voltage tap controller. The master's AND every follower's
# RatioTapChanger reference the SAME control mRID and carry
# controlEnabled = true in SSH, so a reimport regroups them (the importer
# dedups by the shared TapChangerControl mRID; a follower with
# controlEnabled = false would stay FIXED instead of joining the group).
# The regulated bus travels as the control's Terminal reference; target
# and deadband are written in kV in SSH (the importer divides by the
# terminal bus's vn and halves the deadband).
function _collectTapControlRecs(net::Sparlectra.Net, trafoRecs::Vector{NamedTuple}, claim, notices::Vector{String})
  recs = NamedTuple[]
  recByBranch = Dict{Int,NamedTuple}(Int(r.branchIdx) => r for r in trafoRecs)
  for c in Sparlectra._tap_controllers(net)
    c.enabled || continue
    c.mode == :voltage || continue
    c.control_ratio || continue
    c.target_bus === nothing && continue
    c.target_vm_pu === nothing && continue
    mbr = Sparlectra._find_trafo_branch(net, c.trafo)
    members = Int[Int(mbr.branchIdx)]
    for fname in c.followers
      push!(members, Int(Sparlectra._find_trafo_branch(net, fname).branchIdx))
    end
    memberRtcIds = String[]
    ok = true
    for bidx in members
      r = get(recByBranch, bidx, nothing)
      if r === nothing || r.rtc === nothing
        ok = false
        break
      end
      push!(memberRtcIds, String(r.rtc.rtcId))
    end
    if !ok
      push!(notices, "tap controller of $(c.trafo) not exported (a group member has no exported ratio-tap machinery)")
      continue
    end
    tb = Sparlectra.geNetBusIdx(net = net, busName = c.target_bus)
    # control terminal: a transformer terminal sitting on the target bus
    # (the importer resolves the regulated bus through the terminal's TP
    # node); prefer the master's own terminal
    mrec = recByBranch[Int(mbr.branchIdx)]
    termId = mrec.fromIdx == tb ? mrec.terminalIds[1] : (mrec.toIdx == tb ? mrec.terminalIds[2] : nothing)
    if termId === nothing
      for r in trafoRecs
        if r.fromIdx == tb
          termId = r.terminalIds[1]
          break
        elseif r.toIdx == tb
          termId = r.terminalIds[2]
          break
        end
      end
    end
    if termId === nothing
      push!(notices, "tap controller of $(c.trafo) not exported (no transformer terminal on target bus $(c.target_bus))")
      continue
    end
    vn = Sparlectra.getNodeVn(net.nodeVec[tb])
    push!(
      recs,
      (
        tccId = claim(string("TCC|", c.trafo)),
        name = string(c.trafo, "_TCC"),
        terminalId = String(termId),
        target_kV = c.target_vm_pu * vn,
        deadband_kV = c.deadband_vm_pu * vn * 2.0,
        memberRtcIds = memberRtcIds,
      ),
    )
  end
  return recs
...
function writeEQFile(net::Sparlectra.Net, ctx::CGMESContext, path::AbstractString, created::Dates.DateTime; sc_line_data::Dict{Int,CGMESLineShortCircuit} = Dict{Int,CGMESLineShortCircuit}(), sc_source::Union{Nothing,CGMESShortCircuitData} = nothing)
  ...
  open(path, "w") do io
    ...
    for rec in ctx.trafoRecs
      ...
      if rec.rtc !== nothing
        rt = rec.rtc
        # shared group control (#322): master and followers reference ONE
        # TapChangerControl so a reimport regroups them
        tccRef = nothing
        for cr in ctx.tapCtrlRecs
          if String(rt.rtcId) in cr.memberRtcIds
            tccRef = cr.tccId
            break
          end
        end
        ...
        tccRef === nothing || println(io, "    <cim:TapChanger.TapChangerControl rdf:resource=\"#_$(tccRef)\"/>")
        println(io, "  </cim:RatioTapChanger>")
      end
    end

    # regulated tap groups (#322): the shared TapChangerControl objects;
    # mode and terminal live here in EQ, enabled/target/deadband in SSH
    for cr in ctx.tapCtrlRecs
      println(io, "  <cim:TapChangerControl rdf:ID=\"_$(cr.tccId)\">")
      println(io, "    <cim:IdentifiedObject.name>$(xmlEscape(cr.name))</cim:IdentifiedObject.name>")
      println(io, "    <cim:RegulatingControl.mode rdf:resource=\"$(CIM16_NS)RegulatingControlModeKind.voltage\"/>")
      println(io, "    <cim:RegulatingControl.Terminal rdf:resource=\"#_$(cr.terminalId)\"/>")
      println(io, "  </cim:TapChangerControl>")
    end
  ...
  open(path, "w") do io
    ...
    for rec in ctx.trafoRecs
      ...
      if rec.rtc !== nothing
        ...
        # every group member (master AND followers) carries controlEnabled:
        # the importer keeps controlEnabled = false changers FIXED, so a
        # false follower would fall out of the group on reimport
        member && println(io, "    <cim:TapChanger.controlEnabled>true</cim:TapChanger.controlEnabled>")
        println(io, "  </cim:RatioTapChanger>")
      end
    end
    for cr in ctx.tapCtrlRecs
      println(io, "  <cim:TapChangerControl rdf:about=\"#_$(cr.tccId)\">")
      println(io, "    <cim:RegulatingControl.enabled>true</cim:RegulatingControl.enabled>")
      println(io, "    <cim:RegulatingControl.targetValue>$(fmtVal(cr.target_kV))</cim:RegulatingControl.targetValue>")
      println(io, "    <cim:RegulatingControl.targetDeadband>$(fmtVal(cr.deadband_kV))</cim:RegulatingControl.targetDeadband>")
      println(io, "  </cim:TapChangerControl>")
    end
    for rec in ctx.trafo3Recs
      for endrec in rec.ends
