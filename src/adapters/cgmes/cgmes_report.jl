...
# file: src/adapters/cgmes/cgmes_report.jl
...
offset PER ISLAND and judges the aligned deltas (`dva_aligned`, feeding
`max_dva`/`rms_dva`); the raw `dva` stays in the rows. `va_ref_offset_deg`
reports the offset of the largest island, the one that describes the
delivery against the CGM reference. Aligning every island with the main
island's offset would measure the island reference instead of the state.

The alignment removes REFERENCE differences, never state differences. A
large `dva_aligned` on a bus inside the main island is therefore a real
disagreement with the SV profile and has to be explained from the model,
not from the reference: on the FullGrid test configuration the largest
angle deltas sit at the HVDC converter buses, whose SSH data declare
150 MW drawn at one end and 0 MW injected at the other while
`cgmes_import.hvdc_mode = injections` maps the two ends as independent
fixed injections without DC coupling.
...
function compareWithSV(result::CGMESImportResult)
  ...
  # docstring). Median rather than mean inside each island: robust against a
  # single bus whose SV entry is off.
  median_of(v) = begin
    sorted = sort(v)
    n = length(sorted)
    n == 0 ? 0.0 : (isodd(n) ? sorted[(n + 1) ÷ 2] : (sorted[n ÷ 2] + sorted[n ÷ 2 + 1]) / 2.0)
  end
  # `Sparlectra.` is required: this file is the submodule CGMESImporter and
  ...
  island_of = try
    ...
    # never silent: without the island map the comparison falls back to ONE
    # global offset, which reports secondary islands as tens of degrees off
    @warn "CGMES SV comparison: island detection failed, falling back to a single global angle offset; deltas on secondary islands are then not meaningful" exception = err
    Dict{Int,Int}()
  end
  bus_island(name) = get(island_of, get(net.busDict, name, 0), 0)
  per_island = Dict{Int,Vector{Float64}}()
  for r in raw_rows
    isfinite(r.dva) || continue
    push!(get!(per_island, bus_island(r.bus), Float64[]), r.dva)
  end
  offset_of = Dict(id => median_of(v) for (id, v) in per_island)
  # the reported reference offset stays the one of the LARGEST island: it is
  # the number that describes the delivery against the CGM reference
  main_island = isempty(per_island) ? 0 : argmax(Dict(id => length(v) for (id, v) in per_island))
  va_ref_offset_deg = get(offset_of, main_island, 0.0)
  for r in raw_rows
    off = get(offset_of, bus_island(r.bus), va_ref_offset_deg)
    push!(aligned_rows, (bus = r.bus, vm_pu = r.vm_pu, sv_vm_pu = r.sv_vm_pu, dvm = r.dvm, va_deg = r.va_deg, sv_va_deg = r.sv_va_deg, dva = r.dva, dva_aligned = r.dva - off))
  end
