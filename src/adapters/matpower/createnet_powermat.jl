...
# Date: 01.10.2023
# file: src/adapters/matpower/createnet_powermat.jl
...
The MATPOWER column-index tables (bus, gen, branch) the parser uses.
"""
...
# mpc.sparlectra.links matrix (fbus tbus status) or nothing. hasproperty
# guards keep MatpowerCase values built by older constructors (extension
# NamedTuple without a links field) importable.
function _matpower_sparlectra_links(mpc)::Union{Nothing,Matrix{Float64}}
  hasproperty(mpc, :sparlectra) || return nothing
  sp = getproperty(mpc, :sparlectra)
  sp === nothing && return nothing
  hasproperty(sp, :links) || return nothing
...
function _matpower_sparlectra_tap_changers(mpc)::Union{Nothing,Matrix{Float64}}
  ...
  sp = getproperty(mpc, :sparlectra)
  sp === nothing && return nothing
  hasproperty(sp, :tap_changers) || return nothing
  return getproperty(sp, :tap_changers)
end
...
function createNetFromMatPowerCase(; mpc, log::Bool=false, flatstart::Bool=false, cooldown::Int = 0, q_hyst_pu::Float64 = 0.0, enable_pq_gen_controllers::Bool = true, bus_shunt_model = :admittance, matpower_shift_sign::Real = 1.0, matpower_shift_unit = :deg, matpower_ratio = :normal, tap_changer_model::Symbol = :ideal, reference_vm_pu::Union{Nothing,Float64} = nothing, reference_va_deg::Union{Nothing,Float64} = nothing, matpower_pv_voltage_source = :gen_vg, matpower_pv_voltage_mismatch_tol_pu::Float64 = 1e-4, preallocate_network::Symbol = :auto, preallocate_min_buses::Int = 1000, apply_bus_names::Bool = false, apply_branch_names::Bool = false, apply_branch_kind::Bool = false, import_for001_contingencies::Bool = true, matpower_dcline_mode::Symbol = :pf_injections, profile::Union{Nothing,AbstractDict}=nothing)::Net
  ...
  # Optional mpc.sparlectra.links rows become impedance-less BusLinks (busbar
  # couplers). Applied after bus typing is final because addLink! asserts the
  # linked buses share one bus type and neither is the slack.
  sp_links = _matpower_sparlectra_links(mpc)
  if sp_links !== nothing
    for row in eachrow(sp_links)
      fb = Int(row[1])
      tb = Int(row[2])
      haskey(bus_name_by_orig, fb) || throw(ArgumentError("mpc.sparlectra.links references unknown fbus $(fb)."))
      haskey(bus_name_by_orig, tb) || throw(ArgumentError("mpc.sparlectra.links references unknown tbus $(tb)."))
      addLink!(net = myNet, fromBus = bus_name_by_orig[fb], toBus = bus_name_by_orig[tb], status = Int(row[3]))
    end
  end

  # Optional mpc.sparlectra.tap_changers rows: nameplate tap-changer data
  # standard MATPOWER cannot express. The mpc.branch TAP/SHIFT columns stay
  # the NEUTRAL position; the current steps move the LIVE tap_ratio /
  # phase_shift_deg off it on the same grids the estimation fixes to
  # (ratio: tap = neutral / (1 + n * tap_step); phase: additive degrees).
  # A ratio tap_step of 0 explicitly declares "no ratio tap changer" and
  # keeps that transformer out of the estimator's mass release.
  sp_taps = _matpower_sparlectra_tap_changers(mpc)
  if sp_taps !== nothing
    for row in eachrow(sp_taps)
      k = Int(row[1])
      1 <= k <= length(myNet.branchVec) || throw(ArgumentError("mpc.sparlectra.tap_changers references unknown branch $(k)."))
      br = myNet.branchVec[k]
      br.ratio != 0.0 || throw(ArgumentError("mpc.sparlectra.tap_changers row for branch $(k): not a transformer branch (its TAP column is 0)."))
      tstep = Float64(row[2])
      tminS = Float64(row[3])
      tmaxS = Float64(row[4])
      tcur = Float64(row[5])
      ...
      pmaxS = Float64(row[8])
      pcur = Float64(row[9])
      # one shared tap-nameplate application path (branch.jl); the SCF
      # reader routes through the same helper, so a nameplate means the
      # same thing no matter which format carried it
...
"""
    createNetFromMatPowerFile(; filename, kwargs...) -> Net

Parse a MATPOWER `.m`/`.jl` case file and build the network; accepts the
same options as [`createNetFromMatPowerCase`](@ref).
"""
function createNetFromMatPowerFile(; filename::String,
  ...
  bus_shunt_model = isnothing(bus_shunt_model) ? model_cfg.bus_shunt_model : bus_shunt_model
  ...
  tap_changer_model = isnothing(tap_changer_model) ? model_cfg.tap_changer_model : tap_changer_model
