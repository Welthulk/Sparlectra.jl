...
# Date: 20.5.2026
# file: src/adapters/matpower/matpower_runner.jl
...
function with_powerflow_method(cfg::SparlectraConfig, method::Symbol)::SparlectraConfig
  ...
  return SparlectraConfig(; powerflow = pf2, state_estimation = cfg.state_estimation, matpower = cfg.matpower, matpower_export = cfg.matpower_export, model = cfg.model, performance = cfg.performance, benchmark = cfg.benchmark, runtime = cfg.runtime, diagnostics = cfg.diagnostics, output = cfg.output, control = cfg.control)
end
...
function _matpower_import_auto_profile_scan_skipped_rows(mode::Symbol, mat::MatpowerImportConfig, model::ModelConfig; reason::AbstractString)
  ...
  _push_auto_profile_row!(rows, mode, "model.bus_shunt_model", model.bus_shunt_model, model.bus_shunt_model; safe_to_apply = false, reason = evidence)
  return rows
end
...
function _copy_model_with(model::ModelConfig; kwargs...)
  fields = NamedTuple{fieldnames(ModelConfig)}(getfield.(Ref(model), fieldnames(ModelConfig)))
  return ModelConfig(; fields..., kwargs...)
end

function _copy_config_with(cfg::SparlectraConfig; powerflow = cfg.powerflow, matpower = cfg.matpower, model = cfg.model)
  return SparlectraConfig(; powerflow = powerflow, state_estimation = cfg.state_estimation, matpower = matpower, matpower_export = cfg.matpower_export, model = model, performance = cfg.performance, benchmark = cfg.benchmark, runtime = cfg.runtime, diagnostics = cfg.diagnostics, output = cfg.output, control = cfg.control)
end

function matpower_import_auto_profile(mpc, cfg::SparlectraConfig; mode::Symbol = cfg.model.auto_profile, convention_scan = _matpower_import_auto_profile_convention_scan)
  ...
  convention_rows = try
    ...
    if err isa OutOfMemoryError
      ...
      return (config = cfg, rows = _matpower_import_auto_profile_scan_skipped_rows(mode, mat, model; reason = "convention scan skipped because memory allocation failed"), applied = ())
    end
    rethrow()
  end
  ...
  if fragile_start
    ...
    for (field, rec) in pairs(robust_start)
      ...
      # Auto-profile may recommend solver-start settings, but applying them can
      # change the Newton basin. Keep solver-start overrides conservative unless
      # the user explicitly opted into them.
      _push_auto_profile_row!(rows, mode, "power_flow.start_mode.$(field)", cur, rec; safe_to_apply = false, reason = "large or fragile MATPOWER case; robust imported/DC start is only a recommendation", evidence = "nbus=$(nbus) npv=$(npv) best_residual=$(round(best_score; digits = 8))")
    end
  end
  ...
  if qlimit_risk
    ...
    for (field, rec) in pairs(q_recs)
      ...
      # Explicit user/sidecar settings win over auto-profile recommendations.
      # This prevents Web UI profile detection from silently changing convergence behavior.
      _push_auto_profile_row!(rows, mode, option, cur, rec; safe_to_apply = false, reason = "many PV buses or narrow Q ranges; Q-limit guard change is only a recommendation", evidence = "npv=$(npv) narrow_q_ranges=$(narrow_q)")
    end
  end
...
function print_matpower_import_auto_profile_summary(io::IO, auto_profile_result, cfg::SparlectraConfig)
  ...
  println(io, "  auto_profile = ", cfg.model.auto_profile)
...
function print_matpower_import_auto_profile_effective_options(io::IO, cfg::SparlectraConfig)
  ...
  println(io, "---------------------------------------------")
  println(io, "model.auto_profile: ", cfg.model.auto_profile)
  ...
  println(io, "model.bus_shunt_model: ", cfg.model.bus_shunt_model)
...
function run_matpower_import_auto_profile(mpc, cfg::SparlectraConfig)
  cfg.model.auto_profile === :off && return (config = cfg, rows = NamedTuple[], applied = NamedTuple[])
  return matpower_import_auto_profile(mpc, cfg; mode = cfg.model.auto_profile)
end

"""
    write_matpower_import_auto_profile(io, auto_profile_result, cfg; casefile)

Report the import-convention analysis, at the verbosity
`output.console_auto_profile` asks for. That setting existed with values
`:off`, `:compact` (the default) and `:full` but was read by nobody, so
every run printed the full block: about seventy console lines for a case
where every single check said "keep". A study tool should not narrate its
importer that loudly, least of all in another product's vocabulary.

- `:off` prints nothing.
- `:compact` prints only the rows that actually RECOMMEND a change, plus
  one line saying how many checks were made and kept.
- `:full` prints the original block: option list before, recommendation
  table, and effective options after.
"""
function write_matpower_import_auto_profile(io::IO, auto_profile_result, cfg::SparlectraConfig; casefile::AbstractString = "", verbosity::Union{Nothing,Symbol} = nothing)
  # `verbosity` overrides the console setting for writers that are NOT the
  # console: the matpower_auto_profile.log artifact keeps the full block,
  # because a file is read on purpose while the console is read by everyone.
  # The rule is compact console output by default, with the detail kept in
  # the run artifacts.
  mode = something(verbosity, cfg.output.console_auto_profile)
...
function run_matpower_case(; casefile::AbstractString = "", config_file::AbstractString = "")
  ...
  if auto_profile_result !== nothing && cfg.model.auto_profile_log
    write_matpower_import_auto_profile(stdout, auto_profile_result, cfg; casefile = local_case)
    open(logfile, "a") do io
      write_matpower_import_auto_profile(io, auto_profile_result, cfg; casefile = local_case, verbosity = :full)
...
"""
    run_synthetic_tiled_grid_pf_perf(; config_file, args, outdir)

Generate a tiled synthetic grid and measure the power flow on it; the
performance harness entry.
"""
...
"""
    run_voltage_dependent_control_demo(; config_file, plot_curve)

Run the Q(U)/P(U) controller demonstration case and print its outcome.
"""
