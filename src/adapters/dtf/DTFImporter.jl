...
# file: src/adapters/dtf/DTFImporter.jl
...
"""
Importer for the native DTF (FOR001) format: parser, network construction
and the outage bookkeeping of that format.
"""
module DTFImporter
...
(`model.tap_changer_model` in the central configuration; `nothing` reads
...
function build_net(case::DTFCase; bus_shunt_model = :admittance, legacy_voltage_level_collapse_230kv::Bool = false, transformer_ratio_mode = :neutral_one, tap_changer_model::Union{Nothing,Symbol} = nothing)::Net
  ratio_mode = _normalize_transformer_ratio_mode(transformer_ratio_mode)
  tap_model = tap_changer_model === nothing ? model_config().tap_changer_model : tap_changer_model
