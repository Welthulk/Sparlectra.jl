...
function run_import_case_xxxyyyy_loadflow_example(; casefile::AbstractString = DEFAULT_CASEFILE, method::Symbol = :rectangular, max_ite::Int = 30, tol::Float64 = 1e-6, verbose::Int = 1)
  ...
  net_creation_time = @elapsed begin
    net = createNetFromMatPowerFile(filename = local_case, log = (verbose > 0), flatstart = false, enable_pq_gen_controllers = false, verbose = verbose)
  end

  println("Network creation time: ", round(net_creation_time; digits = 6), " seconds")
